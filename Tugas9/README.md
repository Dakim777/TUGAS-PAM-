...
* Memahami cara kerja Large Language Models (LLMs)[cite: 1].
* Mengintegrasikan Google Gemini API ke dalam aplikasi KMP menggunakan Ktor Client[cite: 1].
* Menerapkan teknik *Prompt Engineering* yang efektif[cite: 1].
* Membangun fitur *AI-powered* dalam aplikasi mobile[cite: 1].
...
* **Format Setting**: Menentukan format output yang jelas untuk memudahkan parsing data[cite: 1].
...
### 4. UI/UX (15%)[cite: 1]
* **Typing Indicator**: Menampilkan animasi loading saat AI sedang memproses terjemahan[cite: 1].
* **Chat Interface**: Menggunakan komponen `ChatBubble` untuk membedakan pesan pengguna dan respon AI[cite: 1].
* **Responsive State**: UI yang responsif dengan feedback visual yang jelas saat terjadi error[cite: 1].

### 5. Code Quality (10%)[cite: 1]
* **Security**: API Key disimpan di `local.properties` dan diakses via `BuildConfig` agar aman[cite: 1].
* **Architecture**: Menerapkan pemisahan logika bisnis (Repository) dan UI (ViewModel) yang rapi[cite: 1].
...
Menampilkan UI saat aplikasi berhasil menangkap dan menampilkan pesan error (seperti Permission Denied 403 atau Not Found 404) secara halus kepada pengguna.
